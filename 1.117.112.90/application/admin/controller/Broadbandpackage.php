...
class Broadbandpackage extends Controller
{
    ...
    public function index()
    {
        ...
        $Feature->TableHeader = array("套餐分类","套餐名称","个人绩效","渠道1绩效","渠道2绩效","支付金额","显示顺序");
        $Feature->Fields = array("BROADBAND_PACKAGE_ID","package_classification","package_name","personal_performance","channel_1_performance","channel_2_performance","amount_of_payment","display_order");
    ...
    public function getByClassification(){
        $postdata = input('post.');
        $PackagesClassification = $postdata['PackagesClassification'];
        $BroadbandPackages = \app\admin\model\BroadbandPackage::where('package_classification','=',$PackagesClassification)->order("display_order")->select();
        return json($BroadbandPackages);
    }
