<?php
    ...
    $DEVICE_LIST = getRow("select * from DEVICE_LIST where DEVICE_ID='359339075000861'");
?>
<!DOCTYPE html>
<html>
<head>
...
<script type="text/javascript">
    function shuaxin(){
        location.href = "OrientationZX.php?v=<?php echo rand();?>";
    }
    $(function(){
    	...
    	$.getJSON("getCliZX.php",function(locadata){
    	    var point = new BMap.Point(<?php echo $DEVICE_LIST['B_LONGITUDE'];?>,<?php echo $DEVICE_LIST['B_LATITUDE'];?>);
    	    map.centerAndZoom(point, 15);  // 初始化地图,设置中心点坐标和地图级别
        	map.addControl(top_right_navigation);
        	var marker = new BMap.Marker(point);  // 创建标注
        	map.addOverlay(marker);               // 将标注添加到地图中
        	$("#ddwsj").text("<?php echo $DEVICE_LIST['POSITIONING_TIME'];?>");
        	$("#diang").text("<?php echo $DEVICE_LIST['ELECTRICITY'];?>");
        	$.each(locadata.lbss,function(i,v){
                ...
                var circle = new BMap.Circle(pt,v.d-130,{strokeColor:"blue", fillColor:"#F00", fillOpacity:0.05, strokeWeight:1});
